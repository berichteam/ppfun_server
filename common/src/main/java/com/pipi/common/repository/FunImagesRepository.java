...
public interface FunImagesRepository extends JpaRepository<FunImages, Long> {
    @Query(value = "update fun_images set fun_id=?1 ,blur=?2 ,description = ?3 ,updated_at =NOW() where id=?4 ", nativeQuery = true)
    @Modifying
    void updateOneWithOutBlur(int funId, int blur, String desc, int id);

    @Query(value = "update fun_images set fun_id=?1 ,blur=?2 ,description = ?3 ,image_blur_url =?4,updated_at =NOW() where id=?5 ", nativeQuery = true)
    @Modifying
